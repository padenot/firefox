...
#define MOZINFERENCE_FUNCTION_LIST(X) \
  ...
  X(struct ggml_context*, ggml_init, (struct ggml_init_params params)) \
  X(void, ggml_free, (struct ggml_context * ctx)) \
  X(struct ggml_tensor*, ggml_new_f32, \
    (struct ggml_context * ctx, float value)) \
  X(struct ggml_tensor*, ggml_mul, \
    (struct ggml_context * ctx, struct ggml_tensor * a, \
     struct ggml_tensor * b)) \
  X(struct ggml_tensor*, ggml_add, \
    (struct ggml_context * ctx, struct ggml_tensor * a, \
     struct ggml_tensor * b)) \
  X(struct ggml_cgraph*, ggml_new_graph, (struct ggml_context * ctx)) \
  X(void, ggml_build_forward_expand, \
    (struct ggml_cgraph * cgraph, struct ggml_tensor * tensor)) \
  X(enum ggml_status, ggml_graph_compute_with_ctx, \
    (struct ggml_context * ctx, struct ggml_cgraph * cgraph, int n_threads)) \
  X(float, ggml_get_f32_1d, (const struct ggml_tensor* tensor, int i))

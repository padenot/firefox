    ...
    for (const file of files) {
      try {
        ...
        const progressWrapper = progressCallback
          ? progressData => {
              const { progress, totalLoaded, total } = progressData;
              ...
              if (progressCallback) {
                try {
                  ...
                  if (now - lastLogTime >= 1000) {
                    const bytesDownloaded = totalLoaded - lastLoggedBytes;
                    ...
                    lazy.console.debug(
                      `Download progress - File: ${file}, Progress: ${progress.toFixed(
                        1
                      )}%, Overall: ${overallProgress}%, Loaded: ${totalLoaded}/${total} bytes, Speed: ${speedMBps} MB/s`
                    );

                    lastLogTime = now;
                    lastLoggedBytes = totalLoaded;
                  }

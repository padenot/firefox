...
PORT = 8766
SERVE_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
...
print(f"Model: http://localhost:{PORT}/cstr/parakeet-tdt-0.6b-v3-GGUF/main/parakeet-tdt-0.6b-v3-q4_k.gguf", flush=True)
print(f"Audio: http://localhost:{PORT}/kennedy-appolo.opus", flush=True)

httpd = http.server.HTTPServer(("localhost", PORT), CORSHandler)
httpd.serve_forever()

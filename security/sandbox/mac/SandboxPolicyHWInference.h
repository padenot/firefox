...
static const char SandboxPolicyHWInference[] = R"SANDBOX_LITERAL(
  ...
  ; CRITICAL FIX: Allow reading AND writing to plugin-container cache directory
  ; This allows reusing and creating Metal shaders in the plugin-container cache
  (allow file-read* file-write*
    (require-all
      (require-not (vnode-type SYMLINK))
      (subpath (string-append userCacheDir "/org.mozilla.plugincontainer"))))

...
struct parakeet_context * parakeet_init_from_fd_with_params_no_state(int fd, struct parakeet_context_params params) {
    PARAKEET_LOG_INFO("%s: loading model from file descriptor\n", __func__);

    FILE * file = fdopen(fd, "rb");
    if (!file) {
        PARAKEET_LOG_ERROR("%s: failed to open file descriptor %d\n", __func__, fd);
        return nullptr;
    }
    ...
    PARAKEET_LOG_INFO("%s: model size = %zu bytes\n", __func__, file_size);
...
struct parakeet_context * parakeet_init_from_fd_with_params(int fd, struct parakeet_context_params params) {
    parakeet_context * ctx = parakeet_init_from_fd_with_params_no_state(fd, params);
    if (!ctx) {
        return nullptr;
    }

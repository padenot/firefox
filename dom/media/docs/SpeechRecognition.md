...
- `HWInferenceManagerParent`: manager for per-session recognition actors. Holds
  the trusted `ContentParentId` of the content process it was created for;
  exposed via `ContentId()` so `SpeechRecognitionParent` can attribute install
  requests to their true requester.
- `SpeechRecognitionParent`: runs in the Utility process (its manager,
  `PHWInferenceManager`, is `[ParentProc=Utility, ChildProc=Content]`).
...
- `ContentParent::RecvRequestHWInferenceConnection`: brokers a direct endpoint
  for `PHWInferenceManager` content↔utility IPC channel. This is the only
  speech-recognition-related method on `ContentParent`; consent and install
  are handled entirely in `HWInference` (see below), not here.
...
[Security](/toolkit/components/ml/docs/HWInference.md), using speech
...
Speech recognition always uses `HWINFERENCE_CONTENT_INSTANCE_KEY`, so it never
shares an OS process with browser-triggered inference; see [A flexible
user:process relationship](/toolkit/components/ml/docs/HWInference.md).
...
[HWInference](/toolkit/components/ml/docs/HWInference.md); what matters here is
...
Closing that actor also releases this content process' keep-alive on the
HWInference process itself, which is what ultimately shuts the process down;
...
closes immediately, which is what tests use. This grace period is a speech
recognition policy and lives here, not in `UtilityProcessManager`: other
HWInference consumers may want a different one.
...
docs](/toolkit/components/ml/docs/HWInference.md)), and to interact with
ModelHub.
...
sequenceDiagram
  ...
  Note over BE: opens the HWInference connection if not<br/>already open (RequestHWInferenceConnection,<br/>same as start()), omitted below for brevity

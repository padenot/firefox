...
void SpeechRecognitionBackend::StartSpeechRecognitionSession(
  ...
  mSpeechRecognitionChild->SendInit(aLanguage, mPhrases)->Then(
      GetCurrentSerialEventTarget(), __func__,
      [self = RefPtr{this}](bool aSuccess) {
        if (!aSuccess) {
          LOGE("Failed to initialize speech recognition session - likely another session is active");
        ...
        OnIPCThread([promise, languages = std::move(languages)]() mutable {
          ...
          speechChild->SendIsModelAvailable(languages)->Then(
              ...
              [promise, speechChild = RefPtr{speechChild}](bool available) {
                ...
                mozilla::ipc::SpeechRecognitionChild::Send__delete__(speechChild);
                ...
                NS_DispatchToMainThread(NS_NewRunnableFunction(
                    ...
                    [promise, available]() {
                      if (available) {
                        ...
                        promise->MaybeResolve(
                            SpeechRecognitionAvailabilityStatus::Downloadable);
                      }
                    }));
              },
              [promise, speechChild = RefPtr{speechChild}](mozilla::ipc::ResponseRejectReason reason) {
      ...
      [promise, languages = std::move(languages)](bool aSuccess) mutable {
        ...
        OnIPCThread([promise, languages = std::move(languages)]() mutable {
          ...
          speechChild->SendInstallModels(std::move(languages))
              ->Then(
                  GetCurrentSerialEventTarget(), __func__,
                  [promise, speechChild = std::move(speechChild)](bool success) {
                  ...
                  [promise, speechChild = RefPtr{speechChild}](mozilla::ipc::ResponseRejectReason aReason) {

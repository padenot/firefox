...
void SpeechRecognitionParent::ProcessAudioOnBackgroundThread() {
  ...
  while (mThreadRunning.load()) {
    ...
    if (mLib->whisper_full(mWhisperCtx, wparams, pcmf32.data(),
                           (int)pcmf32.size())) {
    ...
    const int nSegments = mLib->whisper_full_n_segments(mWhisperCtx);
      ...
      NS_DispatchToMainThread(NS_NewRunnableFunction(
          ...
          [self = RefPtr{this}, payload = currentLineTranscript, isFinal]() {
            ...
            if (!self->SendOnRecognitionResult(payload, isFinal)) {
              LOGE("Counldn't send recognition result {} (final={}).",
                   payload.get(), isFinal);
